...
function AdminQuestions() {
  ...
  const handleExportCSV = () => {
    const csvData = questions.map((question) => ({
      ...
      Options: Object.keys(question.options).map(key => `${key}: ${question.options[key]}`).join(', '),
      'Correct Option': `${question.correctOption}: ${question.options[question.correctOption]}`,
    }));
  ...
  return (
    <div>
      ...
      >
        ...
        <Table columns={questionColumn} dataSource={questions} />
      </div>
    </div>
  );
}
